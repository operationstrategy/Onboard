...
static CGFloat const kDefaultUnderIconPadding = 64;
static CGFloat const kDefaultUnderTitlePadding = 64;
...
static CGFloat const kActionButtonHeight = 42;
static CGFloat const kActionButtonWidth = 96;
...
- (instancetype)initWithTitle:(NSString *)title body:(NSString *)body image:(UIImage *)image videoURL:(NSURL *)videoURL buttonText:(NSString *)buttonText actionBlock:(action_callback)actionBlock {
	...
	self.contentWidthMultiplier = kContentWidthMultiplier;
	
	// Icon image view
	self.iconImageView = [[UIImageView alloc] initWithImage:image];
	self.iconWidth = image ? image.size.width * 1.25 : kDefaultImageViewSize;
	self.iconHeight = image ? image.size.height * 1.25 : kDefaultImageViewSize;
	...
	NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:self.bodyLabel.text];
	NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc] init];
	[paragraphStyle setAlignment:NSTextAlignmentCenter];
	paragraphStyle.lineSpacing = 18;
	[attributedString addAttribute:NSParagraphStyleAttributeName value:paragraphStyle range:NSMakeRange(0, self.bodyLabel.text.length)];
...
- (void)viewWillLayoutSubviews {
	...
	CGFloat contentWidth = viewWidth * self.contentWidthMultiplier;
	CGFloat xPadding = (viewWidth - contentWidth) / 2.0;
	
	CGFloat bodyContentWidth = viewWidth * 0.7;
	CGFloat bodyXPadding = (viewWidth - bodyContentWidth) / 2.0;
	
	self.titleLabel.frame = CGRectMake(xPadding, self.topPadding, contentWidth, 0);
	[self.titleLabel sizeToFit];
	
	self.titleLabel.frame = CGRectMake(xPadding, self.topPadding, contentWidth, CGRectGetHeight(self.titleLabel.frame));
	
	CGFloat iconImageYOrigin = CGRectGetMaxY(self.titleLabel.frame) + self.underIconPadding;
	
	[self.iconImageView setFrame:CGRectMake((viewWidth / 2.0) - (self.iconWidth / 2.0), iconImageYOrigin, self.iconWidth, viewWidth / 2)];
	
	CGFloat bodyYOrigin = CGRectGetMaxY(self.iconImageView.frame) + self.underTitlePadding;
	
	self.bodyLabel.frame = CGRectMake(bodyXPadding, bodyYOrigin, bodyContentWidth, 0);
	[self.bodyLabel sizeToFit];
	self.bodyLabel.frame = CGRectMake(bodyXPadding, bodyYOrigin, bodyContentWidth, CGRectGetHeight(self.bodyLabel.frame));
